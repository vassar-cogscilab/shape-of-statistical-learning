...
loo_significant <- function(loo_fit) {
  ...
  for (id in sort(unique(loo_fit[["subject_id"]]))) {
    ...
    loo_res[[as.character(id)]] <-
      looi[["model_label"]][abs(looi[["elpd_diff"]]) <= 5*looi[["se_diff"]]]
  }
  return(loo_res[lengths(loo_res) < 4])
}

waic_fit <- function(fit, sub_ids = NULL,
                     models = c("no_learning", "step_learning",
                                "symmetric_logistic_learning",
                                "asymmetric_logistic_learning")) {
  if (is.null(sub_ids)) {
  ...
  out <- data.frame(subject_id = rep(as.integer(sub_ids), each = n_models),
                    ...
                    elpd_diff = double(n_row),
                    se_diff = double(n_row),
                    elpd_waic = double(n_row),
                    se_elpd_waic = double(n_row),
                    p_waic = double(n_row),
                    se_p_waic = double(n_row),
                    waic = double(n_row),
                    se_waic = double(n_row))
  for (i in 1:n_sub_ids) {
    tmp <- fit[[as.character(sub_ids[i])]]
    waics <- list()
    for (m in 1:n_models) {
      waics[[m]] <- loo::waic(loo::extract_log_lik(tmp[[models[m]]]))
    }
    idx <- 1:n_models + (i-1)*n_models
    waicc <- loo::loo_compare(waics)
    out[idx, 2] <- rownames(waicc)
    out[idx, -c(1, 2)] <- waicc
  }
  return(out)
}
...
plot_waic_fit <- function(loo_fit, sub_ids = NULL, save_path = NULL) {
  if(is.null(sub_ids)) {
    sub_ids <- sort(unique(loo_fit[["subject_id"]]))
  }
  n_sub_ids <- length(sub_ids)
  ...
  loo_fit[["elpd_per"]] <- loo_fit[["elpd_waic"]] /
    rep(loo_fit[["elpd_waic"]][seq(1, by = n_models,
                                  length.out = n_sub_ids)],
        each = n_models)
  ...
  print(ggplot(loo_fit) +
    ...
    guides(color = guide_legend(override.aes = list(size = rep(5, n_models)))) +
    coord_cartesian(ylim = c(0, 1)) +
    labs(title = "Waic Estimates for elpd_waic",
         subtitle = paste("as a percent of the highest",
                          "elpd_waic per individual", sep = "\n"),
         x = "Subject ID Number", y = "Percent of Maximum elpd_waic") +
    theme_bw() +
    theme(
      panel.border = element_blank(),
      plot.title = element_text(size = 20,
  ...
  print(ggplot(loo_fit) +
    geom_pointrange(size = 1, shape = 16, alpha = 0.5,
                    aes(x = subject_id, y = p_waic,
                        ymin = p_waic - se_p_waic,
                        ymax = p_waic + se_p_waic,
                        color = model_label)) +
    geom_line(size = 1.15, alpha = 1, linetype = "dashed",
              aes(x = subject_id, y = n_pars, color = model_label)) +
    scale_color_manual(values = colors, labels = models, name = NULL) +
  ...
  if (!is.null(save_path)) {
    ggsave(file = paste0(save_path, "/p_waic.png"),
           width = 16, height = 9)
  }
}
...
plot_post_pred <- function(fit, data, sub_ids = NULL,
  ...
  for (ind in 1:length(sub_ids)) {
    ...
    if ("no_learning" %in% models) {
      df[["y_nol"]] <- c(
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["no_learning"]])[[pred_names[1]]], 2, mean),
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["no_learning"]])[[pred_names[2]]], 2, mean))
      df[["sd_nol"]] <- c(
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["no_learning"]])[[pred_names[1]]], 2, sd),
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["no_learning"]])[[pred_names[2]]], 2, sd))
      print(ggplot(df) +
    ...
    if ("step_learning" %in% models) {
      df[["y_stp"]] = c(
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["step_learning"]])[[pred_names[1]]], 2, mean),
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["step_learning"]])[[pred_names[2]]], 2, mean))
      df[["sd_stp"]] = c(
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["step_learning"]])[[pred_names[1]]], 2, sd),
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["step_learning"]])[[pred_names[2]]], 2, sd))
      print(ggplot(df) +
        geom_ribbon(linetype = "blank", alpha = 0.25, outline.type = NULL,
    ...
    if ("symmetric_logistic_learning" %in% models) {
      df[["y_sym"]] = c(
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["symmetric_logistic_learning"]])[[pred_names[1]]], 2, mean),
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["symmetric_logistic_learning"]])[[pred_names[2]]], 2, mean))
      df[["sd_sym"]] = c(
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["symmetric_logistic_learning"]])[[pred_names[1]]], 2, sd),
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["symmetric_logistic_learning"]])[[pred_names[2]]], 2, sd))
      print(ggplot(df) +
        geom_ribbon(linetype = "blank", alpha = 0.25, outline.type = NULL,
    ...
    if ("asymmetric_logistic_learning" %in% models) {
      df[["y_asy"]] = c(
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["asymmetric_logistic_learning"]])[[pred_names[1]]], 2, mean),
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["asymmetric_logistic_learning"]])[[pred_names[2]]], 2, mean))
      df[["sd_asy"]] = c(
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["asymmetric_logistic_learning"]])[[pred_names[1]]], 2, sd),
        apply(rstan::extract(fit[[as.character(sub_ids[ind])]][["asymmetric_logistic_learning"]])[[pred_names[2]]], 2, sd))
      print(ggplot(df) +
        geom_ribbon(linetype = "blank", alpha = 0.25, outline.type = NULL,
...
plot_model_est <- function(fit, data, sub_ids = NULL,
  ...
  mu_asy <- function(nk, pars) {
    return(
      (pars[["V"]] + pars[["S"]] + pars[["E"]] * exp(-pars[["A"]] * 1:nk) ) *
      (1 - pars[["D"]] /
        ( 1 + pars[["Q"]] * exp(-pars[["L"]] * (1:nk - pars[["H"]]))
          )^(1/pars[["NU"]]) )
    )
  }
  ...
  for (ind in sub_ids) {
    # read and clean data
    yn <- data[data[["subject_id"]] == ind &
               data[["is_predictable"]] == 0, "rt"]/1000
    yl <- data[data[["subject_id"]] == ind &
               data[["is_predictable"]] == 1, "rt"]/1000
    nk <- min(length(yn), length(yl))
    yn <- yn[seq_len(nk)]
    yl <- yl[seq_len(nk)]
    ...
    for (m in models) {
      pars[[m]] <- lapply(rstan::extract(
        fit[[as.character(ind)]][[m]])[names(pars[[m]])], mean)
    }
    ...
    if ("no_learning" %in% models) {
      df[["y_nol"]] <- c(mu_n(nk, pars[["no_learning"]]),
    ...
    if ("step_learning" %in% models) {
      pars[["step_learning"]][["H"]] <- mean(apply(
        rstan::extract(fit[[as.character(ind)]][["step_learning"]])[["H"]],
        1, which.max))
      df[["y_stp"]] <- c(mu_n(nk, pars[["step_learning"]]),
    ...
    if ("symmetric_logistic_learning" %in% models) {
      df[["y_sym"]] <- c(mu_n(nk, pars[["symmetric_logistic_learning"]]),
    ...
    if ("asymmetric_logistic_learning" %in% models) {
      df[["y_asy"]] <- c(mu_n(nk, pars[["asymmetric_logistic_learning"]]),
...
plot_prior_est <- function(ntrials = 100, ndraws = 1000)
{
  V <- rep(rgamma(ndraws, 5, 4), each = ntrials)
  E <- rep(rgamma(ndraws, 2.5, 10), each = ntrials)
  A <- rep(rgamma(ndraws, 2.5, 10), each = ntrials)
  S <- rep(rnorm(ndraws, 0, 0.1), each = ntrials)
  D <- rep(rbeta(ndraws, 5, 1), each = ntrials)
  L <- rep(rgamma(ndraws, 1, 10), each = ntrials)
  H <- rep(rbeta(ndraws, 5, 4), each = ntrials)
  NU <- rep(rgamma(ndraws, 100, 100), each = ntrials)
  # C <- rep(rgamma(ndraws, 100, 100), each = ntrials)
  Q <- rep(rgamma(ndraws, 100, 100), each = ntrials)
  trial <- rep(seq_len(ntrials), ndraws)

  df <- data.frame(
    draw = rep(seq_len(ndraws), each = ntrials),
    trial = trial,
    y_nol = V + S + E*exp(-A*trial),
    y_stp = ifelse(trial < (H*ntrials),
                     V + S + E * exp(-A*trial),
                     (V + S + E * exp(-A*trial)) * (1 - D)),
    y_sym = (V + S + E*exp(-A*trial)) *
            (1 - D/(1 + exp(-L*(trial - H*ntrials)))),
    y_asy = (V + S + E*exp(-A*trial)) *
            (1 - D/((1 + Q*exp(-L*(trial - H*ntrials)))^(1/NU)))
  )

  print(ggplot(df) +
    geom_line(size = 1, alpha = 0.1, color = "#ff4f4f",
              aes(x = trial, y = y_nol, group = draw)) +
    labs(title = "No Learning Model Prior Estimates",
         x = "Trial Number", y = "Response Time (sec)") +
    theme_bw() +
    theme(
      panel.border = element_blank(),
  ...
  print(ggplot(df) +
    geom_line(size = 1, alpha = 0.1, color = "#ff4f4f",
              aes(x = trial, y = y_stp, group = draw)) +
    labs(title = "Step Learning Model Prior Estimates",
         x = "Trial Number", y = "Response Time (sec)") +
    theme_bw() +
    theme(
      panel.border = element_blank(),
  ...
  print(ggplot(df) +
    geom_line(size = 1, alpha = 0.1, color = "#ff4f4f",
              aes(x = trial, y = y_sym, group = draw)) +
    labs(title = "Symmetric Logistic Learning Model Prior Estimates",
         x = "Trial Number", y = "Response Time (sec)") +
    theme_bw() +
    theme(
      panel.border = element_blank(),
  ...
  print(ggplot(df) +
    geom_line(size = 1, alpha = 0.1, color = "#ff4f4f",
              aes(x = trial, y = y_asy, group = draw)) +
    labs(title = "Asymmetric Logistic Learning Model Prior Estimates",
         x = "Trial Number", y = "Response Time (sec)") +
    theme_bw() +
    theme(
      panel.border = element_blank(),
