...
<li><a href="#comp">Formal Model Comparison (just a sample for now)</a>
  <ul>
  <li><a href="#comp-ppf">Posterior Predictive Fits</a></li>
  <li><a href="#comp-bayes">Bayes Factor</a></li>
  <li><a href="#comp-loo">Loo</a></li>
  <li><a href="#fits-real">Stacking Weights</a></li>
  </ul></li>
...
This section will contain some basic model recovery experiments in order to determine each model's flexibility when fitting data that exhibit distinct patterns. In each of the following subsections, we will generate response time data to mimic a specific behavioral pattern. Then we will fit this generated data to each of the models from the previous section in order to demonstrate each model's ability to capture commonly observed behaviors. First we define functions to generate the response time data. We will generate data that simulates the response times for both the first letter in the pattern and the third letter in the pattern; if there is evidence of learning the pattern, we expect to see a decrease in the median response time for the third letter and not for the first letter. Our simulated data will be taken from a Log-Normal distribution whose median follows the log of the logistic function. This is basically the model from the Symmetric Logistic Learning section.
...
First, we will generate response time data to simulate a participant that exhibits no evidence of learning the lexicographical pattern in the sequence of letters. This data will contain the adaptation to the experimental task, as shown by a decrease in response time across the first several trials of the experiment. Following that, however, will be no further changes to the median response time.
...
78 178
Second, we will generate response time data to simulate a participant that exhibits strong evidence of learning the lexicographical pattern in the sequence of letters. This data will not only contain the adaptation to the experimental task, but it will also contain a significant decrease in the median response time of the learned letter halfway through the experiment. To best mimic the actual data, we will use a very sharp learning curve
...
both sporadic return and boredom
86

boredom and sporadic return at the end (perhaps because of boredom) (with learning)
196
