---
title: "Description of the Models We Used"
...
<li><a href="#models">The Models</a>
  <ul>
  <li><a href="#models-no">No Learning</a></li>
  <li><a href="#models-step">Step Learning</a></li>
  <li><a href="#models-sym">Symmetric Logistic Learning</a></li>
  <li><a href="#models-asym">Asymmetric Logistic Learning</a></li>
  </ul></li>
...
The following subsections will define functional forms for $\hat{\mu}$ that enable it to capture the evidence of learning (or lack of evidence) shown by each participant in the study. We will start with the simplest model, and gradually add more features.




## No Learning {#models-no}

This is the most basic model with just an exponential curve to fit the initial adaptation to the task. Assuming no learning occurs, this model is designed to fit the initial adaptation to the experimental task. This is the most basic model that we will test, as it defines the functional form for $\hat{\mu}$ as only a decaying exponential curve with a few parameters for fitting the data. The model for a single subject is:

\begin{equation} \label{eqn:models-no}
\begin{aligned}
rt &\sim \mathcal{LN}(\hat{\mu}, \hat{\sigma^2}),\\[2ex]
\hat{\mu} &= \log\big( V + E \cdot \exp(-A \cdot t) \big);\\[2ex]
\end{aligned}
\nonumber
\end{equation}
where $t$ is the trial iteration, $t \in \{1, \dots, n\}$, and $n$ is the number of trials performed in the experiment by that individual. 




## Step Learning {#models-step}

This includes a drop in the median response time to allow for the evidence of learning. This one fits this data the best because the learning is instantaneous, but it's inflexible for a more general learning process.




## Symmetric Logistic Learning {#models-sym}

This model allows the drop in the median response time to be gradual. This one can approach a step function and offers a good compromise between flexibility/generality and interpretable parameters.
...
## Asymmetric Logistic Learning {#models-asym}

This model allows the drop in the median response time to be not only gradual but also asymmetric. This might be too many parameters to be useful
