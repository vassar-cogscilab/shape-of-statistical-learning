...
model_mcmc<-as.mcmc(jags.result)
...
plotSubjectModel<-function(subject_data,sample_posterior){
  ...
  p<-ggplot()+
    ...
    geom_line(data=model_plot_data %>% group_by(pair,index) %>% filter(is_predictable == 0),aes(x=t,y=rt_predict,group = index), col= "#e41a1c")+
...
hello<-plot_data_2 %>% filter(subject_id == 41) 
